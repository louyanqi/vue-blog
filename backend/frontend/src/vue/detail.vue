<template>
    <div id="main">
        <div v-show="show" class="animated fadeIn article-detail" style="margin-top: 100px">
	...
	export default {
	  ...
	  methods:{
        ...
        getArticleFun:function(id){
            var self = this;
            self.show=false;
            axios.get('http://127.0.0.1:8000/api/article/'+ id + '/?format=json').then(function(response) {self.article = response.data.data;
        ...
        getComment:function(id) {
            var self = this;
            axios.get('http://127.0.0.1:8000/api/comments/?format=json&article_id='+id).then(function(response){
        ...
        postComment:function(comment) {
            var self = this;
            axios.post('http://127.0.0.1:8000/api/comments/?format=json', {
                article_id: self.id,
                comment_user: self.comment_user,
                comment_content: self.comment_content,
                parent: comment.id
            }).then(function(response) {
                self.getComment(self.id);
                self.comment_user = '';
